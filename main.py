...
def getMyPosition(prcSoFar):
    ...
    limmit = 0.01
    coefficients_all = np.array([LinearRegresion(prcSoFar[i,:]) for i in range(nins)])
    """
    coefficients_one_month = np.array([LinearRegresion(prcSoFar[i,-31:]) for i in range(nins)])
    coefficients_one_week = np.array([LinearRegresion(prcSoFar[i,-7:]) for i in range(nins)])
    coefficients_three_day = np.array([LinearRegresion(prcSoFar[i,-4:]) for i in range(nins)])
    """
    longTermAverage = np.array([a[1] for a in coefficients_all])

    longTermAverage[longTermAverage > limmit] = 9999999999
    longTermAverage[longTermAverage < -limmit] = -9999999999
    currentPos = np.array([int(x) for x in longTermAverage])
    '''
    if nt == 500:
        for i in range(nInst):
            plt.plot(range(nt), prcSoFar[i,:], 'bo')
            plt.plot(range(nt),coefficients_all[i,0] + coefficients_all[i,1] * range(nt))
            plt.plot(range(nt), coefficients_one_month[i, 0] + coefficients_one_month[i, 1] * range(nt), 'r')
            plt.plot(range(nt), coefficients_one_week[i, 0] + coefficients_one_week[i, 1] * range(nt), 'g')
            plt.plot(range(nt), coefficients_three_day[i, 0] + coefficients_three_day[i, 1] * range(nt), 'y')
            plt.show()
    '''
    return currentPos
    # Provided Code end
    ...
    for overNumDays in range(1, maxPeriodOfLeading + 1):
        ...
        for leadingStock in range(np.shape(percentChangeInPriceData)[0]):
            for daysLeading in range(1, maxLeadingTime + 1):
                relativeChangeInPrice = percentChangeInPriceData[
                                            leadingStock, -(daysLeading + 1)] / percentChangeInPriceData[:, -1] - 1
    ...
    for overNumDays in range(1, maxPeriodOfLeading + 1):
        ...
        for leadingStock in range(np.shape(percentChangeInPriceData)[0]):
            for daysLeading in range(0, maxLeadingTime + 1):
                if daysLeading == 0:
                    relativeChangeInPrice = percentChangeInPriceData[leadingStock, :] / percentChangeInPriceData[:,
                                                                                        :] - 1
                else:
                    relativeChangeInPrice = percentChangeInPriceData[leadingStock,
                                            :-daysLeading] / percentChangeInPriceData[:, daysLeading:] - 1
                index1 = relativeChangeInPrice <= 0.7
                index2 = relativeChangeInPrice >= -0.7
                relativeChangeInPrice[index1 == index2] = 1
                relativeChangeInPrice[index1 != index2] = 0
...
def LinearRegresion(stockData):
    A = np.array(
        [[len(stockData), sum(x for x in range(len(stockData)))], [0, sum(x ** 2 for x in range(len(stockData)))]])
    A[1, 0] = A[0, 1]
    b = np.array([sum(stockData), sum(stockData[i] * i for i in range(len(stockData)))])
